...
func setup_ready_button(player_id: PlayersManager.PlayerID, activate: bool, show_button: bool) -> void:
    var panel: GuiPlayerPanel = get_player_panel(player_id)
    if not is_instance_valid(panel):
        printerr("Player panel instance invalid for player ", player_id)
        return
    panel.setup_ready_button(activate, show_button)

func update_requirements_container(player_id: PlayersManager.PlayerID) -> void:
    var panel: GuiPlayerPanel = get_player_panel(player_id)
    if not is_instance_valid(panel):
        printerr("Player panel instance invalid for player ", player_id)
        return
    panel.update_requirements_container()
...
func set_requirement_message(player_id: PlayersManager.PlayerID, message: String) -> void:
    ...
    if not is_instance_valid(panel):
        printerr("Player panel instance invalid for player ", player_id)
        return
    panel.set_requirement_message(message)

func clear_requirement_messages(player_id: PlayersManager.PlayerID) -> void:
    var panel: GuiPlayerPanel = get_player_panel(player_id)
    if not is_instance_valid(panel):
        printerr("Player panel instance invalid for player ", player_id)
        return
